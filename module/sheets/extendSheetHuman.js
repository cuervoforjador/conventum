...
export class extendSheetHuman extends ActorSheet {
  ...
  _frameButton(event) {
    ...
    switch(sAction) {
      ...
      case 'compendium':  //Show compendiums
        helperSheets.openCompendium(sItemId, target?.dataset.folder);
      break;
      ...
      case 'selectItem':
        this.actor.update({
           "system.control.selection.item": sItemId });
      break;
      case 'holding':
        helperSheets.holdingPay(this.actor, target?.dataset);
      break;
      case 'receive':
        helperSheets.payTransfer(this.actor, target?.dataset.amount, false, game.i18n.localize('common.profession'), null, true);
      break;      
      case 'newMonth':
        helperSheets.newMonth(this.actor);
      break;
    }    
  }
  ...
  _frameRightButton(event) {
    ...
    switch(sAction) {
      ...
      break;    
      case 'selectItem':
        this.actor.items?.get(sItemID).sheet.render(true);
      break;
    }
  }
